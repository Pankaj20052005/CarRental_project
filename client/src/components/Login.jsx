...
const login = ({ setShowLogin }) => {
    ...
    return (
        // Added justify-center to center the form horizontally
        <div onClick={() => setShowLogin(false)} className='fixed top-0 bottom-0 left-0 right-0 z-[100] flex items-center justify-center text-sm text-gray-600 bg-black/50'>
            ...
            {/* Corrected e.stopPropagation() typo to prevent form from closing when clicked inside */}
            <form onSubmit={handleSubmit} onClick={(e) => e.stopPropagation()} className="sm:w-[350px] w-full text-center border border-gray-300/60 rounded-2xl px-8 bg-white">
                ...
                <p className="text-gray-500 text-sm mt-3 mb-11">
                    ...
                    <span onClick={() => setState(state === "login" ? "register" : "login")} className="text-indigo-500 hover:underline cursor-pointer ml-1">click here</span>
                </p>
            </form>
        </div>
    )
